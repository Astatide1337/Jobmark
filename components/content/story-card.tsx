...
export function StoryCard({ article, variant = 'standard', className }: StoryCardProps) {
  ...
  if (variant === 'compact') {
    return (
      <Link href={`/articles/${article.slug}`} className={cn(baseClasses, 'block p-4', className)}>
  ...
  if (variant === 'lead') {
    return (
      <Link
        ...
        className={cn(baseClasses, 'block p-6 sm:p-8', className)}
      >
  ...
  return (
    <Link href={`/articles/${article.slug}`} className={cn(baseClasses, 'block p-5', className)}>
